 ...
 *
 * <p>
 * 팔로우(FOLLOWERS 승인)는 P2 에서 추가된다 — 지금은 FOLLOWERS 밴드도 멤버만 통과한다.
 */
@Component
public class BandAccessGuard {
	...
	public BandAccessGuard(BandRepository bands, BandMemberRepository bandMembers) {
		this.bands = bands;
	...
	public boolean canViewContent(Band band, Long userId) {
		...
		return userId != null && bandMembers.existsByBandIdAndUserId(band.getId(), userId);
	}
