...
### ADR-010. 이메일/비밀번호 로그인 추가 (ADR-008 부분 번복)
- **상태**: 승인됨 (2026-09-03)
- **배경**: ADR-008에서 "카카오만"으로 결정했으나, 카카오 계정이 없거나 원치 않는 사용자를 위해 이메일 로그인을 추가하기로 함
- **결정**:
  - USERS: `kakao_id` nullable화, `password_hash`(BCrypt) · `provider`(KAKAO/LOCAL) 컬럼 추가, `email` unique
  - `POST /api/auth/signup` · `/login` 신규 (성공 시 refresh 쿠키 + access 토큰 즉시 반환)
  - 비밀번호 규칙: 8자 이상 + 영문·숫자 포함
  - **이메일 인증(확인 메일) 없음** — 메일 발송 인프라가 없고 10팀 규모라 생략. 아무 이메일로 즉시 가입 가능
  - 같은 이메일로 카카오 계정이 있어도(현재는 카카오 이메일 미수집이라 실질 발생 X) 이메일 가입 시도는 `409 EMAIL_TAKEN`
  - **비밀번호 분실 복구 미구현** — 메일 인프라 생기면 그때. 현재는 잊으면 카카오 로그인 또는 새 계정
- **결과**: ADR-008의 "이메일 미수집"은 카카오 경로에 한해 유지(카카오 유저는 여전히 email=null). LOCAL 유저만 email/password 보유
