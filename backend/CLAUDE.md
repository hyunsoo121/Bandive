...
- **Phase 6.5 — 배포 전 개선 (2026-09-03~, 브랜치 `feature/10`)**: 사용자 요청 묶음. `V4` media.title, `V5` local login.
  ...
  - ✅ 프로필 사진 (2026-09-09): `V13` `users.avatar_url`. `POST/DELETE /api/auth/me/avatar`(멀티파트 업로드/제거, `StorageService` 재사용 — 밴드 로고와 동일 패턴, 이전 파일 정리). `MeResponse`/`MemberResponse` 에 `avatarUrl` 추가(멤버 목록은 공개 GET 이지만 사진은 밴드 로고처럼 노출 OK). 프론트: `ProfileModal` 사진 섹션, `Avatar` 컴포넌트 `src` 로 멤버목록/사이드바/일정출결에 반영
  - ✅ 위시리스트/합주곡 수정 (2026-09-16): `PATCH /api/songs/{id}` — 제목/아티스트/메모/참고영상/세션 구성, 위시리스트·합주곡 상태 모두 가능, 등록자 본인 또는 관리자만(`SongService.requireProposerOrOwner`). 세션 구성은 등록 시와 같은 "전체 목록" 의미로 받아 늘어난 만큼 빈 슬롯 추가·줄어든 만큼 배정 없는 슬롯부터 삭제, 배정된 슬롯을 넘겨 줄이면 `409 SESSION_SLOT_ASSIGNED`. 프론트: `AddSongModal` 에 `editing` prop 추가해 재사용, 수정 모드에서 세션 스테퍼가 배정 인원 아래로 못 내려가게 클라단 선가드.
  - ✅ 합주곡 탭에서 바로 등록 (2026-09-16): `SongCreateRequest.status`(nullable, 기본 `WISHLIST`) — `CONFIRMED` 로 바로 등록하려면 `confirm()` 과 동일하게 관리자여야 함(`NOT_BAND_OWNER`). 등록 위치(미분류 그룹 맨 끝)도 그 status 기준으로 계산. 프론트: `AddSongModal.defaultConfirmed` — 관리자가 합주곡 탭에서 "곡 추가" 열면 자동으로 켜짐, 버튼·안내 문구도 "합주곡으로 등록"으로 바뀜. 제출 후 탭은 등록된 status 쪽에 그대로 남음.
  - ✅ 밴드 생성 시 로고·배너 업로드 (2026-09-16, 프론트만): `CreateBandModal` 의 "업로드" 박스가 그동안 목업이었던 걸 실제로 연결 — `CropModal` 재사용(로고 원형, 배너 3:1). `AppContext.createBand` 가 `POST /api/bands` 로 밴드를 만든 뒤 받은 id로 기존 `POST /api/bands/{id}/logo`·`.../banner`(이미 있던 엔드포인트, Phase 4-1)를 곧바로 호출 — `currentBandId` 기반 `uploadBandLogo`/`uploadBandBanner` 액션은 아직 그 밴드로 이동 전이라 못 쓰고, `createBand` 안에서 직접 `bandApi.uploadLogo/uploadBanner(band.id, file)` 호출. 백엔드 변경 없음.
  - ✅ 곡 제안자별 필터 (2026-09-16, 프론트만): `SongsPage` 정렬 칩 옆에 "제안자" 칩 추가 — 누르면 밴드 멤버별 현재 탭(위시/합주) 제안 곡수를 보여주는 모달, 멤버 선택하면 그 사람이 제안한 곡만 필터링(`Song.addedByUserId` 기준). 필터 중엔 드래그 순서변경 비활성화(그룹 일부만 보여서 서버가 `SONG_ORDER_MISMATCH` 로 거부하기 때문). 백엔드 변경 없음(이미 있던 `addedByUserId` 그대로 사용).
  - ✅ 일정 등록 날짜/시간 입력 개선 (2026-09-16, 프론트만): `AddScheduleModal` 의 `datetime-local` 단일 input → `date`+`time` 두 input 으로 분리(네이티브 피커가 더 다루기 쉬움) + "오늘/내일/이번 주말/다음 주" 퀵 버튼. 백엔드는 그대로 ISO 문자열 받음, 변경 없음.
  - ✅ 음원 검색 결과 스크롤 (2026-09-16, 프론트만): `AddSongModal` 검색 결과 목록(`.addsong__results`)에 `max-height: 240px; overflow-y: auto` — 결과가 많아도(기본 `app.music.limit=8`) 그 목록만 스크롤되고 아래 세션 구성·참고영상 등 나머지 폼이 밀려나지 않음.
  - ✅ 영상 "링크 아님" → "기타 링크" (2026-09-16, 프론트만): `MediaPage` 의 `platform === 'other'` 배지 문구·스타일을 경고(빨간 배지)에서 `.media__kind` 와 같은 중립 배지로 변경(정보성 표시로 톤 다운).
  - ✅ 사진 자르기 공용 모달 (2026-09-16, 프론트만): `components/CropModal` — 순수 Canvas(외부 라이브러리 없음), 드래그로 위치 이동 + 슬라이더 확대, 항상 프레임을 이미지로 채움(letterbox 없음), 적용 시 고정 해상도 JPEG `File`(정사각 960×960 / 배너 960×320, aspect=3)을 콜백으로 반환 — 업로드 자체는 기존 `uploadAvatar`/`uploadBandLogo`/`uploadBandBanner` 그대로 이어서 호출(백엔드 변경 없음). 적용처: `ProfileModal`(아바타, 원형), `BandSettingsPage`·`HomePage`(로고 원형·배너 3:1, 두 곳 다 같은 업로드 액션을 부르는 별개 진입점이라 각각 연결).
  ...
  - ✅ 게스트 멤버 (2026-09-08): 계정 없이 이름만 있는 밴드 구성원. **곡 세션 배정 + 일정 출결에서만** 참조, 로그인·권한·파트·리더와 무관. 등록/수정/삭제는 **관리자만**. `V10` `band_guests` + `song_parts.assigned_guest_id`(ON DELETE SET NULL) + `attendances.guest_id`(ON DELETE CASCADE), `attendances.user_id` nullable화, 부분 유니크 인덱스 2개 + `CHECK((user_id is not null) <> (guest_id is not null))` (멤버·게스트 배타). `guest/{Guest,GuestRepository,dto,service,controller}` — `GET /api/bands/{id}/guests` 공개, `POST`/`PATCH`/`DELETE .../guests/{id}` 관리자(서비스 레벨 체크, 폴더 도메인과 동일 패턴). 곡: `SongPart.assignedGuest`, `assignPart(…, guestId)` 추가 — `userId`/`guestId` 동시 지정 시 `PART_ASSIGN_AMBIGUOUS` 400, `SongPartResponse` 는 `assignedUserId`/`assignedGuestId`/`assignedName`(rename, 기존 `assignedNickname`). 일정: `POST`/`DELETE /api/schedules/{id}/attendance/guests/{guestId}` 관리자 전용 upsert/삭제, `AttendeeResponse` 에 `guestId`(+`userId` nullable), `ScheduleResponse.from` 의 `myStatus` 루프에 user null-guard. 게스트 삭제 시 배정 해제·출결 삭제는 DB FK 가 처리(서비스는 그냥 `delete`). fetch-join(`findAllForBand`/`findByIdWithDetails`, `AttendanceRepository`)에 `assignedGuest`/`guest` left join 추가.
    **`V12`(⚠️ 다른 세션이 같은 번호 `V11__media_song` 를 만들어 `V11`→`V12` 로 밀림 — 병합 시 재번호 필요할 수 있음)**: `band_guests` 밴드 내 이름 유니크(`uq_band_guest_name`) + `GUEST_NAME_TAKEN` 409(생성·이름변경), `band_guests.session varchar(30)` 추가.
    **세션은 게스트 속성**(per-schedule 아님) — 멤버 페이지 게스트 목록의 "세션" 버튼에서 설정. `Guest.session`(악기 또는 "관객", nullable), `GuestResponse.session`, `PUT /api/bands/{bandId}/guests/{guestId}/session` `{session}` 관리자(빈 값이면 지움).
    일정 게스트 UX: 게스트는 그 일정에 **추가된 사람만** 상세에 표시(전체 등록 목록 X). 게스트는 항상 `ATTENDING` — 참석/불참/미정 토글 없이 "빼기"로만 제외. `POST/DELETE .../attendance/guests/{guestId}` 는 **바디 없음**(추가/삭제만), `AttendeeResponse` 는 `guestId`(+`userId` nullable)만, `ScheduleResponse.from` 의 counts·myStatus 는 게스트 행 제외(멤버만 집계).
    프론트: `GuestPickerModal`(맨 위 "새 게스트 추가" 입력+버튼, 아래 등록 게스트 **세로 리스트** — 클릭하면 일정에 추가), `GuestSessionModal`(칩+직접입력, 멤버 페이지), 멤버 페이지 접이식 "게스트 N명" 섹션(추가 버튼 상단, 행마다 세션 라벨 + 세션/이름/삭제), 일정 상세 "참석 게스트" 섹션은 참여 게스트만 + 세션 라벨(게스트 속성에서). 87 tests green(guest/schedule/song 스위트; 다른 세션 media 테스트 WIP 로 전체 build 는 미확인)
